...
public BaseCodeEditorFragment(String path) {
  ...
  editor.setTypefaceText(ResourcesCompat.getFont(requireContext(), R.font.jetbrains_mono_regular));
			...
			editor.getProps().boldMatchingDelimiters = false;
   ...
   menu.add("Guardar").setOnMenuItemClickListener(item -> {
	...
	private void initCodeView() {
		try {
			...
			editorColorScheme = getColorScheme();
	...
	protected TextMateColorScheme getColorScheme() {
	try {
	if(IdroidApplication.isDarkMode(requireContext())){
	
		IThemeSource iTheme =  IThemeSource.fromInputStream(
requireContext().getAssets().open("textmate/darcula.json"), "darcula.json", null);
	return TextMateColorScheme.create(iTheme);
	
	} else {
	IThemeSource iTheme =  IThemeSource.fromInputStream(
requireContext().getAssets().open("textmate/QuietLight.tmTheme"), "QuietLight.tmTheme", null);
	return TextMateColorScheme.create(iTheme);
